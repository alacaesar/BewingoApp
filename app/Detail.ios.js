...
export default class DetailPage extends React.Component{
  ...
  _openDateSelection = ()=> {
    if(this.state.disabled == true)
    {
      this.setState({ dateSelectionIsOpen: true, selectionIsOpen: false, })
    }
  }
  ...
  _onDateChange = (date) => {
    ...
    console.log(date, date.toLocaleDateString('tr-TR'), Globals.dateToLocalString(date));

    this.setState({
      date: date,
      dateName: 'Teslimat: ' + date.toLocaleDateString('tr-TR'),
    })
  }
  ...
  componentDidMount() {
    ...
    Globals.fetchURL(Globals.URLs.suggestions+item.rqs_id, new FormData(), this._handleSuggestionsResponse);
  ...
  _onSubmit = () => {
    ...
    var data = new FormData();
        ...
        data.append('ofr_delivery_date', this.state.date.toLocaleDateString("tr-TR"));
  ...
  render(){
    ...
    var select = this.state.selectionIsOpen ?
      <View>
        <View style={styles.blueBackground}><Button color='#ffffff' title="Seç" onPress={this._onSelect} /></View>
        <Picker
    ...
    const suggestionSelectionTrigger = this.state.noSuggestion == false ?
                                        <TouchableOpacity activeOpacity={0.8} onPress={this._openSelection}>
                                          <View style={styles.input}>
                                            <Text numberOfLines={1} style={[styles.small, styles.dropDownText]}>{this.state.alternative.crs_package}</Text>
                                          </View>
                                        </TouchableOpacity>
                                        : null;

    var dateSelect = this.state.dateSelectionIsOpen ?
      <View>
        <View style={styles.blueBackground}><Button color='#ffffff'  title="Seç" onPress={this._onDateSelect} /></View>
        <DatePickerIOS
          mode='date'
        ...
        <ScrollView style={styles.one}>
          ...
          <View style={[styles.detailsFormHolder, styles.defaultBackground]}>
            ...
            <View style={styles.container}>
              ...
              {suggestionSelectionTrigger}

              <TextInput editable={this.state.disabled} keyboardType='numeric' style={styles.input} placeholder='Fiyat teklifiniz' value={price} onChange={this._onInputChange} underlineColorAndroid='transparent' onFocus={this._onInputFocus} onBlur={this._onInputBlur}></TextInput>

              <TouchableOpacity activeOpacity={0.8} onPress={this._openDateSelection}>
                <View style={styles.input}><Text numberOfLines={1} style={[styles.small, styles.dropDownText]}>{this.state.dateName}</Text></View>
              </TouchableOpacity>
